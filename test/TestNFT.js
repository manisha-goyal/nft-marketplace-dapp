...
contract("NFT", accounts => {
    ...
    it("should mint an NFT to the first account", async () => {
        const newItemId = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft.json", royalty, { from: accounts[0] });    
	...
	it("should emit a Transfer event when an NFT is minted", async () => {
		const receipt = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft1.json", royalty, { from: accounts[0] });
	...
	it("increases token's id after each mint", async () => {
		let receipt = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft3.json", royalty, { from: accounts[0] });
		let tokenId1 = receipt.logs[0].args.tokenId.toNumber();
	
		receipt = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft4.json", royalty, { from: accounts[0] });
		...
		receipt = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft5.json", royalty, { from: accounts[0] });
    ...
    it("should only allow transfer if eligible", async () => {
        const newItemId = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft6.json", royalty, { from: accounts[0] });
        const tokenId = newItemId.logs[0].args.tokenId.toNumber();
    ...
    it("should not allow transfer if not eligible", async () => {
        const newItemId = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft7.json", royalty, { from: accounts[0] });
        const tokenId = newItemId.logs[0].args.tokenId.toNumber();
	...
	it("should enable and disable transfers correctly", async () => {
		const newItemId = await myNftInstance.mintNFT(accounts[1], "https://example.com/nft8.json", royalty, { from: accounts[0] });
        const tokenId = newItemId.logs[0].args.tokenId.toNumber();
	...
	it("should mint multiple NFTs correctly", async () => {
		...
		const tokenURIs = ["https://example.com/nft9.json", "https://example.com/nft10.json"];
